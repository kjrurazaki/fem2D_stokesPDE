...
def all_prints(model, dict_print):
    ...
    if print_dirichlet == 1:
        ...
        rhs_dir[model.boundary_nodes_velocity, 0] = model.rhs[
            model.boundary_nodes_velocity, 0
        ]
        display_results.plot_field_2D(
            model.coord[: model.lines_A, :],
            model.triang_velocity[:, :3],
        ...
        rhs_dir = np.zeros((model.lines_A, 1))
        rhs_dir[model.boundary_nodes_velocity, 0] = model.rhs[
            [i + model.lines_A for i in model.boundary_nodes_velocity], 0
        ]
        display_results.plot_field_2D(
            model.coord[: model.lines_A, :],
            model.triang_velocity[:, :3],
        ...
        rhs_dir[model.boundary_nodes_pressure, 0] = model.rhs[
            [i + 2 * model.lines_A for i in model.boundary_nodes_pressure], 0
        ]
        display_results.plot_field_2D(
            model.coord[: model.lines_B, :],
            model.triang_pressure[:, :3],
