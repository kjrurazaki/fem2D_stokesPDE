...
def localBasis(model):
    ...
    for iel in range(0, model.Nelem):
        ...
        for inod in range(1,4):
            ...
            Bloc[iel, inod - 1] = (model.coord[nodes[n1 - 1] - 1, 1] - model.coord[nodes[n2 - 1] - 1, 1])
            Cloc[iel, inod - 1] = (model.coord[nodes[n2 - 1] - 1, 0] - model.coord[nodes[n1 - 1] - 1, 0])
...
def phi_funcs(model, iel, x, y):
    ...
    phi_B_x = 27 * (model.Bloc[iel, 0] * phi_2 * phi_3 +
                    model.Bloc[iel, 1] * phi_1 * phi_3 +
                    model.Bloc[iel, 2] * phi_1 * phi_2)
    phi_B_y = 27 * (model.Cloc[iel, 0] * phi_2 * phi_3 +
