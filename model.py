...
class Model():
    ...
    def compute_residual(self):
        ...
        u1_residual = [np.linalg.norm(u1_residual), np.linalg.norm(u1_residual/self.lines_A)]
        u2_residual = self.uh[self.lines_A : 2 * self.lines_A] - self.element.solution.u_2
        u2_residual = [np.linalg.norm(u2_residual), np.linalg.norm(u2_residual/self.lines_A)]
        p_residual = self.p - self.element.solution.sol_p[:self.lines_B]
        u2_residual = [np.linalg.norm(p_residual), np.linalg.norm(p_residual/self.lines_A)]
    ...
    def plot_boundary(self, save_name = None):
        ...
        boundary_1_coords = self.coord[np.sort(self.element.DirNod_velocity.flatten(), axis = 0) - 1]
        ...
        if len(self.element.NeuNod) > 0:
            ...
            x_boundary_2, y_boundary_2 = boundary_2_coords[:, 0], boundary_2_coords[:, 1]
            plt.scatter(x_boundary_2, y_boundary_2, color='blue', label='Neumann boundary')
    ...
    def plot_solution(self, title = True, save_name = None):
        ...
        display_results.plot_field_2D(self.coord[:self.lines_A,:], 
        ...
        display_results.plot_field_2D(self.coord[:self.lines_A,:], 
                                      ...
                                      ax = ax[0,1],
                                      cmap = 'coolwarm')
        display_results.plot_field_2D(self.coord[:self.lines_B,:], 
    ...
    def plot_dirichlet(self, title = True, save_name = None):
        ...
        velocities[self.element.DirNod_velocity.flatten() - 1, 0] = self.element.DirVal_velocity[:, 0]
        velocities[self.element.DirNod_velocity.flatten() - 1, 1] = self.element.DirVal_velocity[:, 1]
        
        pressures = np.zeros((self.lines_B, 1))
        pressures[self.element.DirNod_pressure.flatten() - 1, 0] = self.element.DirVal_pressure[:, 0]
        display_results.plot_field_2D(self.coord[:,:], 
                                      self.triang_velocity[:, :3],
                                      velocities[:, 0], 
                                      nodal = False, 
                                      ax = ax[0,0],
