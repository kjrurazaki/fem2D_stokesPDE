    ...
    for iel in range(0, len(model.triang_velocity)):
        for iloc in range(0, 3):
            ...
            for jloc in range(0, 3):
                ...
                B = (model.Bloc[iel, iloc] * model.Bloc[iel, jloc] + 
                     model.Cloc[iel, iloc] * model.Cloc[iel, jloc]) /(4 * model.Area[iel])
                
                elementalMat[iglob - 1, jglob - 1] = (elementalMat[iglob - 1, jglob - 1] + model.viscosity[iel] * B)
                elementalMat[iglob_A2 - 1, jglob_A2 - 1] = elementalMat[iglob - 1, jglob - 1]
    return elementalMat
        ...
        for iloc in range(0, 4):
            ...
            for jloc in range(0, 4):
                ...
                if iloc != 3 and jloc != 3:
                    elementalMat[iglob - 1, jglob - 1] = (elementalMat[iglob - 1, jglob - 1] + model.viscosity[iel] *
                                                          (model.Bloc[iel, iloc] * model.Bloc[iel, jloc] + 
                                                           model.Cloc[iel, iloc] * model.Cloc[iel, jloc]) / 4 / model.Area[iel])
                elif (iloc != 3 and jloc == 3): 
                    elementalMat[iglob - 1, jglob - 1] = (elementalMat[iglob - 1, jglob - 1] + model.viscosity[iel] *
                                                          (model.Bloc[iel, iloc] * np.sum(model.Bloc[iel, :]) + 
                                                           model.Cloc[iel, iloc] * np.sum(model.Cloc[iel, :])) * 27 / 48 / model.Area[iel])
                elif (jloc != 3 and iloc == 3):
                    elementalMat[iglob - 1, jglob - 1] = (elementalMat[iglob - 1, jglob - 1] + model.viscosity[iel] *
                                                          (model.Bloc[iel, jloc] * np.sum(model.Bloc[iel, :]) + 
                                                           model.Cloc[iel, jloc] * np.sum(model.Cloc[iel, :])) * 27 / 48 / model.Area[iel])
                elif (iloc == 3 and jloc == 3): # both basis functions are bubble functions
                    elementalMat[iglob - 1, jglob - 1] = (elementalMat[iglob - 1, jglob - 1] + model.viscosity[iel] *
                                                          (np.sum(model.Bloc[iel, :]**2) + np.sum([model.Bloc[iel, 0] * model.Bloc[iel, 1],
                                                                                                  model.Bloc[iel, 0] * model.Bloc[iel, 2],
                                                                                                  model.Bloc[iel, 1] * model.Bloc[iel, 2]]) + 
                                                           np.sum(model.Cloc[iel, :]**2) + np.sum([model.Cloc[iel, 0] * model.Cloc[iel, 1],
                                                                                                  model.Cloc[iel, 0] * model.Cloc[iel, 2],
                                                                                                  model.Cloc[iel, 1] * model.Cloc[iel, 2]]))
                                                                                                  * 27**2 / 360 / model.Area[iel])
                elementalMat[iglob_A2 - 1, jglob_A2 - 1] = elementalMat[iglob - 1, jglob - 1]
    return elementalMat
    ...
    for iel in range(0, len(model.triang_velocity)):
        for iloc in range(0, 3):
            ...
            for jloc in range(0, 3):
                ...
                jglob_A2 = jglob + model.lines_A
                if iglob == jglob:
                    massMat[iglob - 1, jglob - 1] = massMat[iglob - 1, jglob - 1] + model.Area[iel] / 6
                elif iglob != jglob:
                    massMat[iglob - 1, jglob - 1] = massMat[iglob - 1, jglob - 1] + model.Area[iel] / 12
        ...
        for iloc in range(0, 4):
            ...
            for jloc in range(0, 4):
                ...
                if iloc != 3 and jloc != 3:
                    ...
                    massMat[iglob - 1, jglob - 1] = massMat[iglob - 1, jglob - 1] + 3 * model.Area[iel] / 20
                else:
                    massMat[iglob - 1, jglob - 1] = massMat[iglob - 1, jglob - 1] + 27**2 * model.Area[iel] / 2520
                massMat[iglob_A2 - 1, jglob_A2 - 1] = massMat[iglob - 1, jglob - 1]
    return massMat
    ...
    for iel in range(0, len(model.triang_velocity)):
        for iloc in range(0, 3):
            ...
            for jloc in range(0, 3):
                jglob = model.triang_velocity[iel, jloc]
                jglob_A2 = jglob + model.lines_A
                divMat[iglob - 1, jglob - 1] = divMat[iglob - 1, jglob - 1] + model.Bloc[iel, jloc] / 6
                divMat[iglob - 1, jglob_A2 - 1] = divMat[iglob - 1, jglob_A2 - 1] + model.Cloc[iel, jloc] / 6
    return divMat
        ...
        for iloc in range(0, 3):
            ...
            for jloc in range(0, 4):
                ...
                if jloc != 3 and iloc != 3:
                    ...
                    divMat[iglob - 1, jglob - 1] = divMat[iglob - 1, jglob - 1] + (model.Bloc[iel, iloc] + 
                                                                                   2 * np.sum(model.Bloc[iel, np.delete([0, 1, 2], iloc)])) * 27 / 120
                    divMat[iglob - 1, jglob_A2 - 1] = divMat[iglob - 1, jglob_A2 - 1] + (model.Cloc[iel, iloc] + 
                                                                                   2 * np.sum(model.Cloc[iel, np.delete([0, 1, 2], iloc)])) * 27 / 120
    return divMat
    ...
    for iel in range(0, len(model.triang_velocity)):
        ...
        for iloc in range(0, 3):
            ...
            for jloc in range(0, 3):
                jglob = model.triang_velocity[iel, jloc]
                jglob_A2 = jglob + model.lines_A
                if (iel + 1) % 4 != 0: # central smaller triangle in mesh h - no common nodes with mesh 2h
                    assert iel in model.triangles_relation[iel_pressure]
                    if iglob in model.triang_velocity[iel, :]: # (model.coord[iglob - 1,:] == model.coord[jglob - 1,:]).all():
                        divMat[iglob - 1, jglob - 1] = divMat[iglob - 1, jglob - 1] + model.Bloc[iel, jloc] / 3
                        divMat[iglob - 1, jglob_A2 - 1] = divMat[iglob - 1, jglob_A2 - 1] + model.Cloc[iel, jloc] / 3
                    else:
                        divMat[iglob - 1, jglob - 1] = divMat[iglob - 1, jglob - 1] + model.Bloc[iel, jloc] / 12
                        divMat[iglob - 1, jglob_A2 - 1] = divMat[iglob - 1, jglob_A2 - 1] + model.Cloc[iel, jloc] / 12
                elif (iel + 1) % 4 == 0: # central smaller triangle in mesh h - no common nodes with mesh 2h
                        divMat[iglob - 1, jglob - 1] = divMat[iglob - 1, jglob - 1] + model.Bloc[iel, jloc] / 6
                        divMat[iglob - 1, jglob_A2 - 1] = divMat[iglob - 1, jglob_A2 - 1] + model.Cloc[iel, jloc] / 6
    return divMat
...
def sup_stabilization(model):
    ...
    for iel in range(0, len(model.triang_velocity)):
        for iloc in range(0, 3):
            ...
            for jloc in range(0, 3):
                jglob = model.triang_velocity[iel, jloc]
                numdiffMat[iglob - 1, jglob - 1] = (numdiffMat[iglob - 1, jglob - 1] + model.delta *
                                                    (model.Bloc[iel, iloc] * model.Bloc[iel, jloc] +
                                                    model.Cloc[iel, iloc] * model.Cloc[iel, jloc]) / (4 * model.Area[iel]))
    return numdiffMat
...
def stiffBuild(model):
    ...
    stiffMat = np.concatenate([stiffMat, np.concatenate([-1 * divMat, zeroMat], axis = 1)], axis = 0)
    
    if model.delta is not None:
        numdiffMat = sup_stabilization(model)
        stiffMat[2 * model.lines_A:, 2 * model.lines_A:] = stiffMat[2 * model.lines_A:, 2 * model.lines_A: ] - numdiffMat
    else:
