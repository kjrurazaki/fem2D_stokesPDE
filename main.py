...
def run_2D(model, method):
    ...
    residual_bc = np.linalg.norm(
        u_dir_velocity[boundary_nodes_velocity, :2]
        - uh_reshaped[boundary_nodes_velocity, :]
    )
    residual_bc += np.linalg.norm(
        u_dir_pressure[boundary_nodes_pressure, 0]
            ...
            for c in c_list:
                for delta in delta_list:
                    if delta is not None and type != "linear_gls":
                        ...
                        uh, p, residual, residual_boundary, rhs = run_2D(
                            model_stokes, method=method
                        )
                        ...
                        model_stokes.update_rhs(
                            rhs
                        )  # After applying boundary condition - to plot
                        ...
                        pd.DataFrame(model_stokes.coord).to_csv(
                            f"./Models/coord_i{i}_type{type}_c{c}_delta{delta}.csv"
                        )
                        pd.DataFrame(model_stokes.triang_velocity).to_csv(
                            f"./Models/triangv_i{i}_type{type}_c{c}_delta{delta}.csv"
                        )
                        pd.DataFrame(model_stokes.triang_pressure).to_csv(
                            f"./Models/triangp_i{i}_type{type}_c{c}_delta{delta}.csv"
                        )
                        pd.DataFrame(model_stokes.uh).to_csv(
                            f"./Models/v_i{i}_type{type}_c{c}_delta{delta}.csv"
                        )
                        pd.DataFrame(model_stokes.p).to_csv(
                            f"./Models/p_i{i}_type{type}_c{c}_delta{delta}.csv"
                        )
                        pd.DataFrame(model_stokes.element.solution.u_1).to_csv(
                            f"./Models/v1sol_i{i}_type{type}_c{c}_delta{delta}.csv"
                        )
                        pd.DataFrame(model_stokes.element.solution.u_2).to_csv(
                            f"./Models/v2sol_i{i}_type{type}_c{c}_delta{delta}.csv"
                        )
                        pd.DataFrame(model_stokes.element.solution.sol_p).to_csv(
                            f"./Models/psol_i{i}_type{type}_c{c}_delta{delta}.csv"
                        )
                        pd.DataFrame(model_stokes.element.DirNod_velocity).to_csv(
                            f"./Models/DirNod_i{i}_type{type}_c{c}_delta{delta}.csv"
                        )
                        pd.DataFrame(model_stokes.element.h).to_csv(
                            f"./Models/h_i{i}_type{type}_c{c}_delta{delta}.csv"
                        )
