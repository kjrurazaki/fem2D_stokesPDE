...
def impose_dirichlet(model, stiffMat, rhs):
    ...
    for idir in range(0, model.element.NDir_pressure):
        ...
        rhs[iglob - 1 + 2 * model.lines_A, :] = (
            -1 * model.element.DirVal_pressure[idir, 0]
        )
...
def impose_neumann(model, rhs):
    ...
    for ineu in range(0, model.NNeu):
        ...
        # Neumann side
        elements_boundary = np.intersect1d(
            np.where(model.triang == iglob)[0], boundary_triangles
        )
        for iel in elements_boundary:
            boundary_loc = np.isin(model.triang[iel], boundary_nodes)
            iloc = np.where(model.triang[iel] == iglob)[0][
                0
            ]  # find the location of the node
            jloc = np.nonzero((model.triang[iel] != iglob) & boundary_loc)[0][
                0
            ]  # find the location of the other node in the boundary
            jglob = model.triang[iel, jloc]
            p1 = model.coord[model.triang[iel, iloc] - 1, :].reshape(
                1, -1
            )  # coordinate of first node
            p2 = model.coord[model.triang[iel, jloc] - 1, :].reshape(
                1, -1
            )  # coordinate of second node

            # Calculate the length of the line segment
            length = np.linalg.norm(p2 - p1)
            rhs[iglob - 1, :] = rhs[iglob - 1, :] + length / 4 * (
                model.NeuVal[ineu, 0]
                + model.NeuVal[np.where(model.NeuNod == jglob)[0], 0]
            )
            rhs[iglob - 1 + model.size_A1, :] = rhs[
                iglob - 1 + model.size_A1, :
            ] + length / 4 * (
                model.NeuVal[ineu, 1]
